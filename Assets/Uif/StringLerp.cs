...
namespace Uif {
	public sealed class StringLerp {
		...
		public string Lerp(float t) {
			...
			if (useNumeric) return NumericLerp(t);
			else if (useDifference) return DifferenceLerp(t);
			else {
...
#region Calculations
		...
		static Edit[] EditPath(int[] costs, string target) {
			...
			while (i > 0 || j > 0) {
				...
				if (sub <= ins && sub <= del) {
					if (costs[n * i + j] != costs[n * (i - 1) + j - 1]) path.Add(new Edit(SUB, i - 1, target[j - 1].ToString()));
					--i;
					--j;
				} else if (ins <= del) {
		...
		static int[] CostMatrix(string source, string target, int ins = -1, int del = -1, int sub = -1) {
			...
			var d = new int[m * n];
			int i, j;
			for (i = 1; i < m; ++i) d[n * i] = i;
			for (j = 1; j < n; ++j) d[j] = j;
			for (j = 1; j < n; ++j) for (i = 1; i < m; ++i) if (source[i - 1] == target[j - 1]) d[n * i + j] = d[n * (i - 1) + j - 1];
					else d[n * i + j] = Mathf.Min(
							del + d[n * (i - 1) + j],
							Mathf.Min(
								ins + d[n * i + j - 1],
								sub + d[n * (i - 1) + j - 1]));
