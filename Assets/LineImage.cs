...
namespace UnityEngine.UI {
	...
	public class LineImage : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter {
		...
		public virtual void OnAfterDeserialize() {
			if (m_FillOrigin < 0) m_FillOrigin = 0;
			else if (m_FillMethod == FillMethod.Horizontal && m_FillOrigin > 1) m_FillOrigin = 0;
			else if (m_FillMethod == FillMethod.Vertical && m_FillOrigin > 1) m_FillOrigin = 0;
			else if (m_FillOrigin > 3) m_FillOrigin = 0;
		...
		void GenerateSimpleSprite(VertexHelper vh, bool lPreserveAspect) {
			...
			var color32 = color;
			vh.Clear();
			vh.AddVert(new Vector3(v.x + EndOffset, v.y), color32, new Vector2(uv.x, uv.y));
			vh.AddVert(new Vector3(v.x + StartOffset, v.w), color32, new Vector2(uv.x, uv.w));
			vh.AddVert(new Vector3(v.z - StartOffset, v.w), color32, new Vector2(uv.z, uv.w));
			vh.AddVert(new Vector3(v.z - EndOffset, v.y), color32, new Vector2(uv.z, uv.y));
		...
		void GenerateTiledSprite(VertexHelper toFill) {
			...
			if (tileWidth <= 0) tileWidth = xMax - xMin;
...

			{
				if (m_FillAmount < 1f && m_FillMethod != FillMethod.Horizontal && m_FillMethod != FillMethod.Vertical) {
					if (fillMethod == FillMethod.Radial90) {
						if (RadialCut(s_Xy, s_Uv, m_FillAmount, m_FillClockwise, m_FillOrigin)) AddQuad(toFill, s_Xy, color, s_Uv);
						...
						for (int corner = 0; corner < 4; ++corner) {
							...
							if (RadialCut(s_Xy, s_Uv, Mathf.Clamp01(val), m_FillClockwise, ((corner + 2) % 4))) AddQuad(toFill, s_Xy, color, s_Uv);
						}
					}
				} else {
					AddQuad(toFill, s_Xy, color, s_Uv);
		...
		public virtual float preferredWidth {
			get {
				...
				if (type == Type.Sliced || type == Type.Tiled) return Sprites.DataUtility.GetMinSize(activeSprite).x / pixelsPerUnit;
		...
		public virtual float preferredHeight {
			get {
				if (activeSprite == null) return 0;
				if (type == Type.Sliced || type == Type.Tiled) return Sprites.DataUtility.GetMinSize(activeSprite).y / pixelsPerUnit;
		...
		private Vector2 MapCoordinate(Vector2 local, Rect rect) {
			...
			if (type == Type.Simple || type == Type.Filled) return new Vector2(local.x * spriteRect.width / rect.width, local.y * spriteRect.height / rect.height);

			Vector4 border = activeSprite.border;
			Vector4 adjustedBorder = GetAdjustedBorders(border / pixelsPerUnit, rect);

			for (int i = 0; i < 2; i++) {
				if (local[i] <= adjustedBorder[i]) continue;
